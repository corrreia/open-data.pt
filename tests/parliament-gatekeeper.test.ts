...
import {
  collectNormalized, libraryConfig, NORMALIZED_PROTOCOL,
  type CollectionRequest, type JsonObject, type NormalizedFrame, type SourceFetch,
...
import {
  collectParliamentFeed, parliamentDirectoryLink, parliamentDocument, parliamentDocumentLink, PARLIAMENT_HTML_BYTES, validateParliamentFeedConfig,
...
interface PublicDirectoryFixture { landing: string; folder: string; download: string }
function links(doc: ParliamentDocument, token = "c3ludGhldGljLXBhdGg="): PublicDirectoryFixture {
  const folder = new URL(doc.pageUrl); folder.searchParams.set("t", "abcdef0123456789"); folder.searchParams.set("Path", token);
  const download = new URL("https://app.parlamento.pt/webutils/docs/doc.txt"); download.searchParams.set("path", token); download.searchParams.set("fich", doc.filename); download.searchParams.set("Inline", "true");
  ...
  return {
    ...
    folder: `<a title="${doc.filename}" href="${escaped(download.toString())}">JSON</a>`, download: download.toString(),
  };
}
function fixture(feed: string): string { return readFileSync(new URL(`./fixtures/parliament/${feed}.json`, import.meta.url), "utf8"); }
...
function sourceFetcher(doc: ParliamentDocument, status = 200, token?: string): typeof fetch {
  ...
  return async (input) => {
    ...
    return status === 304 ? new Response(null, { status: 304 }) : new Response(chunked(fixture(doc.feed)), { headers: { ETag: '"document-v1"', "Last-Modified": "Tue, 15 Sep 2026 12:00:00 GMT" } });
  };
}
async function consume(source: SourceFetch): Promise<void> { if (source.kind === "body") await new Response(source.body).arrayBuffer(); }
...
describe("Parliament public-directory source", () => {
  it("requires a supported feed and explicit legislature with no arbitrary URL, host or credentials", () => {
    ...
    for (const invalid of [{ feed: "members" }, { ...config, legislature: "current" }, { ...config, legislature: "XVIII" }, { ...config, feed: "private" }, { ...config, url: "https://attacker.example" }, { ...config, downloadUrl: links(document).download }, { ...config, host: "app.parlamento.pt:8443" }]) expect(() => validateParliamentFeedConfig(invalid)).toThrow();
  });
  ...
  it("discovers two levels, decodes entities, and ignores the publisher's HTTP canonical URL", async () => {
    const calls: URL[] = []; const inner = sourceFetcher(document);
    const result = await collectParliamentFeed(config, undefined, async (input, init) => {
      calls.push(new URL(input.toString())); expect(init?.redirect).toBe("manual"); return inner(input, init);
    });
    ...
    expect(result).toMatchObject({ kind: "body", completeness: "complete", provenance: { sourceUrl: document.pageUrl, sourcePublishedAt: "2026-09-15T12:00:00.000Z" }, state: { resource: document.filename, validators: { default: { etag: '"document-v1"' } } } });
  ...
  it("checks every directory/download boundary rather than trusting an anchor title", () => {
    const directoryUrls = ["https://attacker.example/Cidadania/Paginas/DAInformacaoBase.aspx?t=ab&Path=x", "http://www.parlamento.pt/Cidadania/Paginas/DAInformacaoBase.aspx?t=ab&Path=x", "https://www.parlamento.pt/Cidadania/Paginas/Other.aspx?t=ab&Path=x", "https://user@www.parlamento.pt/Cidadania/Paginas/DAInformacaoBase.aspx?t=ab&Path=x", `${document.pageUrl}?t=ab&Path=x&Path=y`, `${document.pageUrl}?t=not-hex&Path=x`, `${document.pageUrl}?t=ab&Path=x#fragment`];
    ...
    const invalid = [download.replace("app.parlamento.pt", "attacker.example"), download.replace("https:", "http:"), download.replace("app.parlamento.pt", "app.parlamento.pt:8443"), download.replace("app.parlamento.pt", "user:password@app.parlamento.pt"), download.replace("doc.txt", "other.txt"), download.replace(document.filename, "InformacaoBaseXVI_json.txt"), `${download}&fich=${document.filename}`, download.replace("Inline=true", "Inline=false"), "javascript:alert(1)"];
  ...
  it("scopes conditional requests to the logical file even when encrypted links refresh", async () => {
    const first = await collectParliamentFeed(config, undefined, sourceFetcher(document)); await consume(first);
    if (first.kind !== "body") throw new Error("Expected body");
    const calls: Headers[] = []; const inner = sourceFetcher(document, 304, "bmV3LXN5bnRoZXRpYy10b2tlbg=");
    const unchanged = await collectParliamentFeed(config, first.state, async (input, init) => { calls.push(new Headers(init?.headers)); return inner(input, init); });
    expect(unchanged).toEqual({ kind: "not-modified", validator: { etag: '"document-v1"', lastModified: "Tue, 15 Sep 2026 12:00:00 GMT" } });
    expect(calls[0]?.has("if-none-match")).toBe(false); expect(calls[1]?.has("if-none-match")).toBe(false);
    ...
    await consume(await collectParliamentFeed(config, wrongScope, async (input, init) => { expect(new Headers(init?.headers).has("if-none-match")).toBe(false); return normal(input, init); }));
  });
  ...
  it("rejects unsolicited304s and redirects and clears validators a source stops supplying", async () => {
    ...
    await expect(collectParliamentFeed(config, undefined, async () => new Response(null, { status: 302, headers: { Location: "https://attacker.example" } }))).rejects.toMatchObject({ code: "source-denied" });
  ...
  it("bounds directory and source bodies and forwards upstream Retry-After", async () => {
    await expect(collectParliamentFeed(config, undefined, async () => new Response("x", { headers: { "Content-Length": String(PARLIAMENT_HTML_BYTES + 1) } }))).rejects.toMatchObject({ code: "response-too-large" });
    await expect(collectParliamentFeed(config, undefined, async () => { throw new TypeError("Network unavailable"); })).rejects.toMatchObject({ code: "upstream-error" });
    await expect(collectParliamentFeed(config, undefined, async () => new Response(null, { status: 503, headers: { "Retry-After": "120" } }))).rejects.toMatchObject({ code: "upstream-error", retryAfterSeconds: 120 });
    const inner = sourceFetcher(document);
    await expect(collectParliamentFeed(config, undefined, async (input, init) => new URL(input.toString()).hostname === "app.parlamento.pt" ? new Response("x", { headers: { "Content-Length": String(document.sourceBytes + 1) } }) : inner(input, init))).rejects.toMatchObject({ code: "response-too-large" });
    const fetched = await collectParliamentFeed(config, undefined, async (input, init) => new URL(input.toString()).hostname === "app.parlamento.pt" ? new Response(new Uint8Array(document.sourceBytes + 1)) : inner(input, init));
  ...
  it.each(PARLIAMENT_EXAMPLES)("streams $slug through the complete protocol-v4 collector", async (example) => {
    const raw = libraryConfig(example.config); const doc = parliamentDocument(raw);
    const collector = parliamentCollector({ config: raw, fetcher: sourceFetcher(doc) });
    const resolved = await collector.resolve(raw);
    const request: CollectionRequest = { protocol: NORMALIZED_PROTOCOL, collectionId: `fixture_${doc.feed}`, feed: { id: "fixture", slug: example.slug, title: example.title, description: example.description }, resolved, feedEpoch: "fixture", observedAt: "2026-09-15T00:00:00Z", deadline: new Date(Date.now() + 30_000).toISOString(), mode: { kind: "live" }, limits: { sourceBytes: doc.sourceBytes, outputBytes: 8 * 1024 * 1024, recordBytes: 256 * 1024, frameBytes: 272 * 1024, records: 50_000, products: 16 } };
    ...
    for await (const frame of readFrames(result.stream, request.limits, { collectionId: request.collectionId, resourceKey: resolved.resourceKey, configHash: resolved.configHash, feedEpoch: request.feedEpoch, mode: request.mode, deadline: request.deadline })) frames.push(frame);
