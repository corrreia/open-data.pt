...
describe("internet infrastructure live research", () => {
  for (const example of [...RIPESTAT_EXAMPLES, ...PEERINGDB_EXAMPLES]) {
    it.skipIf(!selected.includes(example.slug) && !selected.includes(example.config.source ?? ""))(example.slug, async () => {
      let requests = 0;
      let detail = "";
      ...
      console.log(JSON.stringify({ slug: example.slug, requests, ...complete.counts, normalizedBytes: new TextEncoder().encode(frames.map((frame) => JSON.stringify(frame)).join("\n")).length }));
    }, 90_000);
  }

  it.skipIf(!selected.includes("ripestat"))("RIPEstat source-supported historical window", async () => {
