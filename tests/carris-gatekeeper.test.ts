...
import {
  collectCarrisFeed,
  validateCarrisFeedConfig,
} from "../packages/gatekeeper-shared/src/sources/carris/carris";

describe("Carris Gatekeeper", () => {
  it("rejects unsupported feed names", () => {
    expect(() => validateCarrisFeedConfig({ feed: "arbitrary-url" })).toThrow(
      "feed=lines, routes, stops, vehicles, or alerts",
    );
  });
  ...
  it("keeps requests on the fixed API origin, forwards checkpoints, and types the body's provenance", async () => {
    ...
    const fetched = await collectCarrisFeed(
      { feed: "vehicles" },
      { lastModified: "Sun, 06 Sep 2026 22:59:04 GMT" },
  ...
  it("fails as an upstream error when the API does not answer 2xx", async () => {
    ...
    await expect(
      collectCarrisFeed({ feed: "alerts" }, undefined, "https://api.carrismetropolitana.pt", fetcher),
    ).rejects.toMatchObject({ name: "GatekeeperError", code: "upstream-error" });
  });
});
