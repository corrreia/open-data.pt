...
import {
  NORMALIZED_PROTOCOL, collectNormalized, libraryConfig,
  type CollectionRequest, type ExampleFeed, type NormalizedCollector,
...
async function request(example: ExampleFeed): Promise<CollectionRequest> {
  ...
  return {
    protocol: NORMALIZED_PROTOCOL, collectionId: `history-probe-${example.slug}`, feed: { id: "probe", slug: example.slug, title: example.title, description: example.description },
    resolved: await collector.resolve(libraryConfig(example.config)), feedEpoch: "probe", mode: { kind: "live" },
    limits: { sourceBytes: policy.maxBytes, outputBytes, recordBytes, frameBytes: Math.min(outputBytes, recordBytes + 16 * 1024), records: policy.maxRecords ?? 1_000_000, products: 64 },
    deadline: new Date(Date.now() + policy.timeoutSeconds * 1000).toISOString(), observedAt: new Date().toISOString(),
  };
}
...
describe.skipIf(examples.length === 0)("new source automatic-history smoke checks", () => {
  it.each(examples)("walks one older slice of $slug", async (example) => {
    const collector = sourceCollector(example);
    ...
    const liveScope = { collectionId: live.collectionId, resourceKey: live.resolved.resourceKey, configHash: live.resolved.configHash, feedEpoch: live.feedEpoch, mode: live.mode, deadline: live.deadline };
    ...
    const historical: CollectionRequest = { ...live, collectionId: `older-${example.slug}`, mode: { kind: "history", cursor: { before } }, deadline: new Date(Date.now() + example.policy.collection.timeoutSeconds * 1000).toISOString() };
    const result = await collectNormalized(historical, collector);
    if (result.kind === "exhausted") { console.info(`${example.slug}: exhausted before ${before}`); return; }
    if (result.kind !== "batch") throw new Error(`History source returned ${JSON.stringify(result)}`);
    const scope = { collectionId: historical.collectionId, resourceKey: historical.resolved.resourceKey, configHash: historical.resolved.configHash, feedEpoch: historical.feedEpoch, mode: historical.mode, deadline: historical.deadline };
    let complete = false;
    let rows = 0;
