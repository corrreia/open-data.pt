...
describe("REN Gatekeeper", () => {
  it("normalizes valid configurations and rejects malformed service or day values", () => {
    ...
    expect(() => validateRenFeedConfig({ service: "unknown" })).toThrow(
      "REN feeds require service=",
    );
    expect(() =>
      validateRenFeedConfig({ service: "consumption", day: "2026-02-30" }),
    ).toThrow("valid date");
  });

  it("selects yesterday and, after 06:00 Lisbon time, today", () => {
    expect(defaultCollectionDays(new Date("2026-01-15T05:59:00Z"))).toEqual([
      "2026-01-14",
    ]);
    expect(defaultCollectionDays(new Date("2026-01-15T06:00:00Z"))).toEqual([
      "2026-01-14",
      "2026-01-15",
    ]);
    expect(defaultCollectionDays(new Date("2026-07-15T04:59:00Z"))).toEqual([
      "2026-07-14",
    ]);
    expect(defaultCollectionDays(new Date("2026-07-15T05:00:00Z"))).toEqual([
      "2026-07-14",
      "2026-07-15",
    ]);
  });
  ...
  it("rejects caller-provided hosts and non-allowlisted deployment origins", async () => {
    ...
    await expect(
      collectRenFeed(
        { service: "consumption", day: "2026-09-06" },
        undefined,
        "https://evil.example",
        vi.fn(),
  ...
  it("posts the verified query contract and returns provenance and a content validator", async () => {
    const fetcher = vi.fn(async (input: URL | RequestInfo, init?: RequestInit) => {
      ...
      expect(url.pathname).toBe(
        "/service/Electricity/ProductionBreakdown/1354",
      );
      expect(url.searchParams.get("culture")).toBe("en-GB");
      expect(url.searchParams.get("dayToSearchString")).toBe(
        dotNetTicks("2026-09-06"),
      );
    ...
    const fetched = sourceBody(await collectRenFeed(
      { service: "production-breakdown", day: "2026-09-06" },
      undefined,
  ...
  it("uses the synthetic content ETag as a checkpoint and reports not modified", async () => {
    ...
    const first = sourceBody(await collectRenFeed(
      config,
      undefined,
      "https://datahub.ren.pt",
      fetcher,
    ...
    const second = await collectRenFeed(
      config,
      { etag },
      "https://datahub.ren.pt",
      fetcher,
  ...
  it("reports an upstream 304 as not modified and forwards Last-Modified checkpoints", async () => {
    ...
    const fetched = await collectRenFeed(
      { service: "consumption", day: "2026-09-06" },
  ...
  it("maps provider failures to an upstream error carrying the retry delay", async () => {
    const fetcher = vi.fn(async () =>
      jsonResponse({ message: "unavailable" }, { status: 503, headers: { "Retry-After": "90" } }),
    );
    await expect(
      collectRenFeed(
        { service: "gas-consumption", day: "2026-09-06" },
        undefined,
        "https://datahub.ren.pt",
        fetcher,
      ),
...
describe("REN history", () => {
  ...
  it("collects one older civil day with its range, next cursor, and transform-compatible bytes", async () => {
    const fetcher = vi.fn(async (input: URL | RequestInfo) => {
      const url = new URL(input.toString());
      expect(url.searchParams.get("dayToSearchString")).toBe(
        dotNetTicks("2026-09-05"),
      );
      return jsonResponse(completeChart());
    });
    const before = "2026-09-05T23:00:00.000Z";
    const fetched = sourceBody(await collectRenHistory(
      { service: "consumption" },
      { before },
      "https://datahub.ren.pt",
      fetcher,
    ...
    const transformed = new RenTransformer().transform(
      fetched.body,
      transformContext("consumption"),
    );
    expect(transformed.products[0]?.points).toHaveLength(96);
    expect(
      transformed.products[0]?.points?.every((point) => point.eventTime < before),
    ).toBe(true);
  });
  ...
  it("requires two consecutive empty source days before reporting exhaustion", async () => {
    const noData = {
      message:
        '<div><p>There is no data<br>for the selected date</p></div>',
    };
    const fetcher = vi.fn(async () => jsonResponse(noData, { status: 400 }));
    const first = sourceBody(await collectRenHistory(
      { service: "consumption" },
      { before: "2026-09-05T23:00:00.000Z" },
      "https://datahub.ren.pt",
      fetcher,
    ));
    ...
    const second = await collectRenHistory(
      { service: "consumption" },
      next,
      "https://datahub.ren.pt",
      fetcher,
  ...
  it("reports exhaustion without another source request past the probed earliest day", async () => {
    const fetcher = vi.fn();
    const fetched = await collectRenHistory(
      { service: "gas-consumption" },
      { before: "2014-01-01T00:00:00.000Z" },
      "https://datahub.ren.pt",
      fetcher,
    );
  ...
  it.each([429, 503])(
    "maps history HTTP %i to a retryable upstream error with its retry delay",
    async (status) => {
      const fetcher = vi.fn(async () =>
        jsonResponse({ message: "retry later" }, { status, headers: { "Retry-After": "30" } }),
      );
      await expect(
        collectRenHistory(
          { service: "gas-network-balance" },
          { before: "2026-09-05T23:00:00.000Z" },
          "https://datahub.ren.pt",
          fetcher,
        ),
