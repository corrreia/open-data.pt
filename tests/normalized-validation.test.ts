...
describe("resolved feed validation", () => {
  ...
  ])("rejects malformed descriptor %j", (value) => expect(() => assertResolvedFeed(value)).toThrow());
});
...
describe("collection result envelope validation", () => {
  it("accepts only exact mode-compatible result envelopes", () => {
    ...
    expect(() => assertCollectionResult({ kind: "unchanged", checkpoint }, live)).not.toThrow();
    expect(() => assertCollectionResult({ kind: "unchanged", checkpoint }, history)).toThrow();
  ...
  it.each([
    ...
    { kind: "unchanged", checkpoint: header().checkpoint, extra: true },
    { kind: "exhausted", checkpoint: header().checkpoint },
  ])("rejects malformed or extended RPC result %j", (result) => {
    expect(() => assertCollectionResult(result, { kind: "live" })).toThrow();
  });
});
