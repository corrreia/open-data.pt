...
function historyResponse(url: URL, earliest = "2004-01-01T00:00:00"): JsonObject {
  ...
  data.query_starttime = url.searchParams.get("starttime"); data.query_endtime = url.searchParams.get("endtime");
  data.earliest_time = earliest; data.stats = [];
  return root;
}
...
describe("RIPEstat capabilities and boundaries", () => {
  it("accepts only named capabilities, PT country scopes and bounded AS numbers/windows", () => {
    ...
    for (const config of [
      { feed: "arbitrary", country: "PT" }, { ...RESOURCES, country: "ES" }, { ...STATUS, asn: "0" },
      { ...STATUS, asn: "4294967296" }, { ...STATUS, asn: "64496/path" }, { ...STATUS, host: "internal.test" },
      { ...ROUTING, days: "0" }, { ...ROUTING, days: "91" }, { ...ROUTING, days: "2.5" }, { ...RESOURCES, asn: "64496" },
  ...
  it("constructs one bounded daily query rather than the endpoint's all-history default", async () => {
    ...
    const fetcher: typeof fetch = async (input, init) => {
      observed = new URL(String(input)); expect(init?.redirect).toBe("manual");
  ...
  it("rejects alternate origins, credentials, paths, and redirects without following them", async () => {
    ...
    let requests = 0;
    const fetcher: typeof fetch = async (_input, init) => { requests += 1; expect(init?.redirect).toBe("manual"); return new Response(null, { status: 302, headers: { Location: "https://evil.test" } }); };
  ...
  it("preserves Retry-After and bounds declared source bytes", async () => {
    await expect(collectRipestatFeed(STATUS, undefined, RIPESTAT_ORIGIN, async () => new Response("busy", { status: 429, headers: { "Retry-After": "600" } }))).rejects.toMatchObject({ code: "upstream-error", retryAfterSeconds: 600 });
    await expect(collectRipestatFeed(STATUS, undefined, RIPESTAT_ORIGIN, async () => new Response("{}", { headers: { "Content-Length": String(RIPESTAT_MAX_BYTES + 1) } }))).rejects.toMatchObject({ code: "response-too-large" });
  });
  ...
  it("binds validators to the exact URL and never uses a prior window's validators", async () => {
    ...
    const fetcher: typeof fetch = async (_input, init) => { expect(new Headers(init?.headers).get("if-none-match")).toBe('"source"'); return new Response(null, { status: 304 }); };
    expect(await collectRipestatFeed(STATUS, state, RIPESTAT_ORIGIN, fetcher)).toMatchObject({ kind: "not-modified" });
    const changed: typeof fetch = async (_input, init) => { expect(new Headers(init?.headers).has("if-none-match")).toBe(false); return Response.json(networkFixture("ripestat-routing")); };
  ...
  it("walks real availability bounds, advancing empty gaps without inventing exhaustion", async () => {
    ...
    await expect(collectRipestatFeed(ROUTING, undefined, RIPESTAT_ORIGIN, fetcher, { kind: "history", cursor: { before: "bad" } }, NOW)).rejects.toMatchObject({ code: "invalid-config" });
    await expect(collectRipestatFeed(ROUTING, undefined, RIPESTAT_ORIGIN, fetcher, { kind: "history", cursor: { before: "2026-02-31T00:00:00Z" } }, NOW)).rejects.toMatchObject({ code: "invalid-config" });
  });
});
...
describe("RIPEstat normalized data", () => {
  it("streams all resource families in one-byte chunks without acquisition or query-time churn", async () => {
    ...
    document.time = "2040-01-01T00:00:00"; document.query_id = "another"; object(document.data).query_time = "2039-12-31T00:00:00";
    ...
    for (const row of first.rows) { expect(row.record?.eventTime).toBeUndefined(); expect(row.record?.sourcePublishedAt).toBeUndefined(); }
  ...
  it("uses RIS snapshot time and real IPv6 /48 units without claiming customer outages", async () => {
    ...
    document.time = "2040-01-01T00:00:00"; document.query_id = "another";
  ...
  it("excludes the endpoint's inclusive upper boundary from the exclusive serving/history window", async () => {
    const root = networkFixture("ripestat-routing"); const data = object(root.data);
    const sample: JsonObject = { timeline: [{ starttime: "2026-09-16T00:00:00", endtime: "2026-09-16T00:00:00" }], v4_prefixes_ris: 9, v6_prefixes_ris: 9, asns_ris: 9, asns_stats: 9, stats_date: "2026-09-16T00:00:00" };
    data.stats = [sample];
  ...
  it("rejects API errors, foreign resources, wrong resolution, incomplete families and invalid source values", async () => {
    const error = networkFixture("ripestat-status"); error.status = "error";
    await expect(transform(STATUS, error)).rejects.toMatchObject({ code: "invalid-response" });
    const wrongAs = networkFixture("ripestat-status"); object(wrongAs.data).resource = "64497";
    await expect(transform(STATUS, wrongAs)).rejects.toMatchObject({ code: "invalid-response" });
    const wrongCountry = networkFixture("ripestat-routing"); object(wrongCountry.data).resource = "ES";
    await expect(transform(ROUTING, wrongCountry)).rejects.toMatchObject({ code: "invalid-response" });
    const hourly = networkFixture("ripestat-routing"); object(hourly.data).resolution = "1h";
    await expect(transform(ROUTING, hourly)).rejects.toMatchObject({ code: "invalid-response" });
    const absent = networkFixture("ripestat-resources"); delete object(object(absent.data).resources).ipv6;
    await expect(transform(RESOURCES, absent)).rejects.toMatchObject({ code: "invalid-response" });
    const invalid = networkFixture("ripestat-resources"); object(object(invalid.data).resources).ipv4 = ["999.1.1.1/24"];
    await expect(transform(RESOURCES, invalid)).rejects.toMatchObject({ code: "invalid-response" });
    const clock = networkFixture("ripestat-status"); object(clock.data).query_time = "2026-02-31T16:00:00";
  ...
  it("enforces resource, status-document and configured timeline bounds", async () => {
    ...
    const oversized = networkFixture("ripestat-status"); oversized.padding = "x".repeat(65 * 1024);
  ...
  it("downgrades provider warnings instead of authorizing an empty authoritative retraction", async () => {
    const root = networkFixture("ripestat-resources"); root.messages = [["warning", "partial source"]];
