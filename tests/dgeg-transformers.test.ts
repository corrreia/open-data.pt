...
import { DgegTransformer } from "../packages/gatekeeper-shared/src/sources/dgeg/transform";
  ...
  return {
    feed: {
      ...
      semantics: {
        ...
        cadence: feed === "fuel-prices" ? "periodic" : "slow-changing",
...
describe("DGEG transformers", () => {
  ...
  it("is deterministic and rejects malformed station rows without losing valid rows", () => {
    const value = JSON.parse(new TextDecoder().decode(fixture("fuel-prices-lisbon.json")));
