...
describe("internal R2 SQL client", () => {
  ...
  it("surfaces provider errors as a rejected query", async () => {
    await expect(runLakeQuery(env, "SELECT 1 LIMIT 1", "test", async () => Response.json({ success: false, errors: [{ message: "failure" }] }, { status: 500 }))).rejects.toThrow(QueryError);
  });
  ...
  it("says why a query failed: its own guard, the store, an unreachable store, or an unreadable answer", async () => {
    ...
    await expect(query(async () => Response.json({ success: false, errors: [{ message: "failure" }] }, { status: 500 }))).rejects.toMatchObject({ failure: "store", message: "failure" });
    await expect(query(async () => { throw new TypeError("network down"); })).rejects.toMatchObject({ failure: "store" });
  ...
  it("accepts one bounded read statement only", () => {
    ...
    for (const sql of ["", "DELETE FROM t LIMIT 1", "SELECT 1", "SELECT 1 LIMIT 5000", "SELECT 1 LIMIT 1; SELECT 2 LIMIT 1", "SELECT 'unclosed LIMIT 1", "SELECT 'a;' AS x LIMIT 1; DELETE FROM t LIMIT 1"]) {
