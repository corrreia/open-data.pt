...
export default {
  async fetch(request: Request, env: StagingEnv): Promise<Response> {
    ...
    return Response.json({ digest, bytes: stored.byteLength, sha: [...new Uint8Array(await crypto.subtle.digest("SHA-256", stored))].map((byte) => byte.toString(16).padStart(2, "0")).join("") });
  },
};
