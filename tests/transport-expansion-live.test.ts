...
const selected = process.env.LIVE_TRANSPORT_EXPANSION?.split(",").map((value) => value.trim()).filter(Boolean) ?? [];
...
describe.skipIf(selected.length === 0)("live transport and municipal expansion", () => {
  it.each(examples.filter((example) => selected.includes("all") || selected.includes(example.slug)))("collects $slug within its actual policy", async (example) => {
    ...
    const fetcher: typeof fetch = async (input, init) => {
      ...
      return new Response(response.body.pipeThrough(new TransformStream<Uint8Array, Uint8Array>({
        transform(chunk, controller) { sourceBytes += chunk.byteLength; controller.enqueue(chunk); },
      })), { status: response.status, headers: response.headers });
    };
    const collector: NormalizedCollector = example.config.source === "gtfs"
    ...
    let outputBytes = 0;
    let maximumFrame = 0;
    const stream = result.stream.pipeThrough(new TransformStream<Uint8Array, Uint8Array>({
      transform(chunk, controller) { outputBytes += chunk.byteLength; maximumFrame = Math.max(maximumFrame, chunk.byteLength); controller.enqueue(chunk); },
    }));
    for await (const frame of readFrames(stream, request.limits, {
      collectionId: request.collectionId, resourceKey: resolved.resourceKey, configHash: resolved.configHash,
      feedEpoch: request.feedEpoch, mode: request.mode, deadline: request.deadline,
    })) {
    ...
    clocks.sort();
    console.info(JSON.stringify({ slug: example.slug, sourceBytes, outputBytes, maximumFrame, counts: Object.fromEntries(counts), firstTime: clocks[0], lastTime: clocks.at(-1) }));
...
function requestFor(example: ExampleFeed, resolved: CollectionRequest["resolved"]): CollectionRequest {
  ...
  return {
    protocol: NORMALIZED_PROTOCOL, collectionId: `live_${example.slug}`, feed: { id: "fixture", slug: example.slug, title: example.title, description: example.description },
    resolved, feedEpoch: "live", observedAt: new Date().toISOString(), mode: { kind: "live" }, deadline: new Date(Date.now() + policy.timeoutSeconds * 1000).toISOString(),
    limits: { sourceBytes: policy.maxBytes, outputBytes: policy.maxOutputBytes ?? 16 * 1024 * 1024, records: policy.maxRecords ?? 1_000_000, recordBytes: policy.maxRecordBytes ?? 256 * 1024, frameBytes: (policy.maxRecordBytes ?? 256 * 1024) + 16 * 1024, products: 64 },
  };
}
