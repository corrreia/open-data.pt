...
const live = process.env.LIVE_PARLIAMENT?.split(",").map((value) => value.trim()).filter(Boolean) ?? [];
...
async function collect(example: ExampleFeed, fetcher: typeof fetch, mode: "saved-research" | "live"): Promise<void> {
  ...
  const request: CollectionRequest = {
    protocol: NORMALIZED_PROTOCOL, collectionId: `${mode}_${config.feed}`, feed: { id: "validation", slug: example.slug, title: example.title, description: example.description },
    resolved, feedEpoch: "validation", mode: { kind: "live" }, observedAt: new Date().toISOString(), deadline: new Date(Date.now() + policy.timeoutSeconds * 1000).toISOString(),
    limits: { sourceBytes: policy.maxBytes, outputBytes: policy.maxOutputBytes ?? 8 * 1024 * 1024, recordBytes: 256 * 1024, frameBytes: 272 * 1024, records: policy.maxRecords ?? 50_000, products: 16 },
  };
  ...
  const stream = result.stream.pipeThrough(new TransformStream<Uint8Array, Uint8Array>({ transform(chunk, controller) { outputBytes += chunk.byteLength; maximumFrame = Math.max(maximumFrame, chunk.byteLength); controller.enqueue(chunk); } }));
  for await (const frame of readFrames(stream, request.limits, { collectionId: request.collectionId, resourceKey: resolved.resourceKey, configHash: resolved.configHash, feedEpoch: request.feedEpoch, mode: request.mode, deadline: request.deadline })) {
  ...
  expect(counts.get("header")).toBe(1); expect(counts.get("complete")).toBe(1); expect(counts.get("record")).toBeGreaterThan(0);
}
...
describe.skipIf(!samples)("Parliament saved research normalization", () => {
  it.each(PARLIAMENT_EXAMPLES)("validates the complete recorded $slug source with aggregate-only output", async (example) => {
    ...
    const fetcher: typeof fetch = async (input) => {
      ...
      const name = url.hostname === "www.parlamento.pt"
        ? `${url.search ? "parl2_" : "parl_"}${document.page}.html`
        : `parlj_${document.filename}`;
...
describe.skipIf(live.length === 0)("Parliament live public-directory collection", () => {
  it.each(PARLIAMENT_EXAMPLES.filter((example) => live.includes("all") || live.includes(example.config.feed ?? "") || live.includes(example.slug)))("collects $slug live", async (example) => {
