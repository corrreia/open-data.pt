...
packages/gatekeeper-<topic>/                       generated by `pnpm packages:sync`; never edited
```
...
A Worker contains no parsing, and nobody writes one: `packages/gatekeeper-<topic>/src/index.ts` is generated to call `topicGatekeeper(topic, deployments, examples)` from `@open-data-pt/gatekeeper-shared/topic-worker`, which builds each library from the Worker's environment through its `LibraryDeployment`, owns the examples whose first topic is the Worker's, and routes. Routing is one rule: every example configuration carries `source: "<library>"`; `resolveTopicFeed` and `topicCollector` pick the library from it, and the library validates the rest of the configuration and never sees that key. `listFeedKinds()` returns the union of the Worker's libraries' kinds, each renamed `<library>:<kind>`; a resolved resource key is `<topic>:<library>:<kind>:<digest>`.
...
- A feed runs in the Worker of its **first** topic, which must be a key of `TOPICS` (`packages/gatekeeper-shared/src/topics.ts`). Put the topic the feed is most about first; later entries are browsing tags. Moving a feed is a change to its `topics`, never to a Worker.
- `pnpm packages:sync` generates each `packages/gatekeeper-<topic>/` (`src/index.ts`, `wrangler.jsonc`, `package.json`, `tsconfig.json`) from the feeds' first topics and the `worker.ts` of every library they use: vars with their values, secrets, R2 buckets, and the largest CPU limit. `tests/packages-sync.test.ts` fails when a checked-in file differs, and runs the generator under plain Node, so shared code must use erasable TypeScript only (no parameter properties, enums or namespaces).
- A new topic is one key in `TOPICS` plus a feed that names it first; `pnpm packages:sync` creates the package, then `pnpm install` and `pnpm types`.
- Telecommunications data (INE's telecom surveys, RIPEstat routing, PeeringDB exchanges) is `gatekeeper-telecom`.
- A source under a publication hold (`packages/gatekeeper-shared/src/publication-holds.json`: RIPEstat, PeeringDB) is still wired into the Worker of its examples' first topic, but those examples are not installed until the owner lifts the hold.
...
Most work is a library, not a Worker. Add a directory under `formats/` or `sources/`, export the surface above, and declare in `worker.ts` one namespaced var (`<LIBRARY>_ALLOWED_HOSTS` for an allowlist, `<LIBRARY>_API_ORIGIN` for a fixed origin) plus any secrets, R2 buckets and CPU limit, and how the library is built from that environment. Its value is written once; every Worker that carries the library gets it from `pnpm packages:sync`.

A new dataset from a source already read is one entry in that library's `examples.ts`, then `pnpm packages:sync`. Its first topic decides the Worker; if that Worker did not carry the library, it does after the sync.

## Implement the Worker

Workers are generated (see One Worker per topic). `topicGatekeeper` returns the default export: a `WorkerEntrypoint` with the five `FeedGatekeeper` operations that returns 404 from public HTTP.
...
`topicGatekeeper` implements `resolveFeed` with `resolveTopicFeed` and `collect` with `collectNormalized(request, topicCollector(...))`. A library's `<name>Collector(options)` returns the `NormalizedCollector` those drive: `{ normalizer, resolve, source, normalize }`.
...
`pnpm packages:sync` generates the topic Workers, the root scripts and the kernel's `GATEKEEPER_*` service bindings, and `tests/packages-sync.test.ts` fails when the checked-in files drift from the generator. After it creates a Worker, run `pnpm install` and `pnpm types`. `pnpm dev -- <topic>` runs the kernel with that Worker alone; the Gatekeepers that are not running log `gatekeeper_unavailable` and their feeds are left alone.
