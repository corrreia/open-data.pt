...
description: Create, modify, or review open-data.pt Gatekeeper libraries and their Workers. Use when adding a public data source, adding a format or source library, changing the Gatekeeper RPC contract, wiring a GATEKEEPER_* binding, or working on source synchronization behavior.
---

# Writing an open-data.pt Gatekeeper

A Gatekeeper is one Cloudflare Worker per library — `arcgis`, `bpstat`, `carris`, `ckan`, `dgeg`, `eurostat`, `gbfs`, `gtfs`, `ine`, `ipma`, `metrolisboa`, `ogc`, `omie`, `opendatasoft`, `parliament`, `ren`, `udata` — reached through private RPC. Together they own upstream access, parsing, validation, normalization, source clocks, validators, coverage, and source-supported history. They never return original source bytes to the kernel and own no canonical storage.
...
packages/gatekeeper-shared/src/sources/<name>/     carris  metrolisboa  ipma  dgeg  ine  ren  omie  bpstat  eurostat  parliament  myinfo  ripestat  peeringdb
packages/gatekeeper-<library>/                     generated by `pnpm packages:sync`; never edited
```
...
A Worker contains no parsing, and nobody writes one: `packages/gatekeeper-<library>/src/index.ts` is generated to call `libraryGatekeeper(deployment, examples)` from `@open-data-pt/gatekeeper-shared/library-worker`, which builds the library from the Worker's environment through its `LibraryDeployment`, owns that library's examples, and routes. Its `describe()` is `{ kind: <library>, name: <the deployment's name> }`. Routing is one rule: every example configuration carries `source: "<library>"`; `resolveTopicFeed` and `topicCollector` pick the library from it, and the library validates the rest of the configuration and never sees that key. `listFeedKinds()` returns the library's kinds, each renamed `<library>:<kind>`; a resolved resource key is `<library>:<library>:<kind>:<digest>`.

## One Worker per library

A Worker is **how** the data is read, never what it is about or who publishes it. Topics overlap — a municipal Wi-Fi map is `cities` and `telecom` — and a publisher may be read two ways, so neither can place a Worker; the library can, because a feed is read exactly one way. A future browser-rendering library, for instance, needs a different compatibility flag, CPU limit and cost profile from a JSON API, and as its own Worker it gets them without touching anything else.

- Topics are catalog tags: any number, any order, each a key of `TOPICS` (`packages/gatekeeper-shared/src/topics.ts`). A test rejects a tag outside that vocabulary, and a new tag is one key in `TOPICS`. Tags never move a feed between Workers.
- `pnpm packages:sync` generates each `packages/gatekeeper-<library>/` (`src/index.ts`, `wrangler.jsonc`, `package.json`, `tsconfig.json`) from that library's `worker.ts`: its name, vars with their values, secrets, R2 buckets and CPU limit. `tests/packages-sync.test.ts` fails when a checked-in file differs, and runs the generator under plain Node, so shared code must use erasable TypeScript only (no parameter properties, enums or namespaces).
- A new library is its directory plus `worker.ts` and one example; `pnpm packages:sync` creates its Worker package, then `pnpm install` and `pnpm types`, and the kernel picks up the new `GATEKEEPER_*` binding.
- A source under a publication hold (`packages/gatekeeper-shared/src/publication-holds.json`: RIPEstat, PeeringDB, MYINFO) gets **no** Worker and installs no examples; lifting the hold and running `pnpm packages:sync` creates it.

Feed slugs never change: a feed's ID derives from its slug, so a feed keeps its history wherever it runs.
...
5. `packages/gatekeeper-shared/src/library.ts` (the deployment declaration and the routing every Worker uses)
...
7. one generated Worker, `packages/gatekeeper-ckan/src/`, which is one library and nothing else
...
Most work is a library, not a Worker. Add a directory under `formats/` or `sources/`, export the surface above, and declare in `worker.ts` the library's human `name`, one namespaced var (`<LIBRARY>_ALLOWED_HOSTS` for an allowlist, `<LIBRARY>_API_ORIGIN` for a fixed origin) plus any secrets, R2 buckets and CPU limit, and how the library is built from that environment. Its value is written once; `pnpm packages:sync` puts it in the library's Worker.

A new dataset from a source already read is one entry in that library's `examples.ts`, then `pnpm packages:sync`. It runs in that library's Worker; its `topics` are only catalog tags.

## Implement the Worker

Workers are generated (see One Worker per library). `libraryGatekeeper` returns the default export: a `WorkerEntrypoint` with the five `FeedGatekeeper` operations that returns 404 from public HTTP.
...
`libraryGatekeeper` implements `resolveFeed` with `resolveTopicFeed` and `collect` with `collectNormalized(request, topicCollector(...))`. A library's `<name>Collector(options)` returns the `NormalizedCollector` those drive: `{ normalizer, resolve, source, normalize }`.
...
`pnpm packages:sync` generates the library Workers, the root scripts and the kernel's `GATEKEEPER_*` service bindings, and `tests/packages-sync.test.ts` fails when the checked-in files drift from the generator. After it creates a Worker, run `pnpm install` and `pnpm types`. `pnpm dev -- <library>` (for example `pnpm dev -- ckan`) runs the kernel with that Worker alone; the Gatekeepers that are not running log `gatekeeper_unavailable` and their feeds are left alone.
