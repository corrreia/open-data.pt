...
- A source under a publication hold (`packages/gatekeeper-shared/src/publication-holds.json`: RIPEstat, PeeringDB) is wired into its topic Worker's `libraries()` with its vars, but its examples stay out of that Worker's `examples.ts` until the owner lifts the hold. Every library must be wired into some Worker; the consistency test checks that too.
