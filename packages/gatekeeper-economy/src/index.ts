...
import { ECONOMY_EXAMPLES } from "./examples";
 ...
 */
export default class EconomyGatekeeper extends WorkerEntrypoint<Env> implements FeedGatekeeper {
  ...
  async describe(): Promise<GatekeeperDescription> {
    return { kind: "economy", name: "Economy and finance" };
  }
  ...
  async exampleFeeds(): Promise<ExampleFeed[]> {
    return ECONOMY_EXAMPLES;
  }

  private topic(): TopicOptions {
    return { gatekeeperKind: "economy", libraries: this.libraries() };
  }
