 ...
 */
export default class EnergyGatekeeper extends WorkerEntrypoint<Env> implements FeedGatekeeper {
      ...
      [
        "ren",
        {
          ...
          collector: (config: SourceConfig) =>
            renCollector({ config, apiOrigin: this.env.REN_API_ORIGIN, dataApiOrigin: this.env.REN_DATA_API_ORIGIN, fetcher: (input, init) => fetch(input, init) }),
        },
      ],
      [
        "omie",
        {
          ...
          collector: (config: SourceConfig) => omieCollector({ config, apiOrigin: this.env.OMIE_API_ORIGIN, fetcher: (input, init) => fetch(input, init) }),
        },
      ],
      [
        "dgeg",
        {
          ...
          collector: (config: SourceConfig) => dgegCollector({ config, apiOrigin: this.env.DGEG_API_ORIGIN, fetcher: (input, init) => fetch(input, init) }),
        },
      ],
      [
        "opendatasoft",
