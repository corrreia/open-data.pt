...
const SERIES_SCHEMA: CanonicalSchema = {
  fields: [
    ...
    field("eventTime", "datetime", false),
    field("value", "number", false, "vehicle"),
    ...
    return [
      {
        ...
        payload: {
          ...
          wheelchairBoarding:
            isJsonBoolean(value.wheelchair_boarding) ? value.wheelchair_boarding : null,
...
function transformVehicles(
    ...
    const record: CanonicalRecord = {
        ...
        payload: {
          ...
          wheelchairAccessible:
            isJsonBoolean(value.wheelchair_accessible)
              ? value.wheelchair_accessible
              : null,
          bikesAllowed:
            isJsonBoolean(value.bikes_allowed) ? value.bikes_allowed : null,
        },
    };
  ...
  const latestEventTime = records
    .map((record) => record.eventTime)
    .filter((value): value is string => Boolean(value))
    .sort()
    .at(-1) ?? context.observedAt;
  const measuredAt = new Date(
    Math.floor(new Date(latestEventTime).getTime() / 60_000) * 60_000,
  ).toISOString();
...
function transformAlerts(values: JsonValue[]): UnstampedResult {
  const records = values.flatMap((value) => {
    ...
    const periods = Array.isArray(value.active_period)
      ? value.active_period.filter(isJsonObject)
      : [];
    const starts = periods
      .map((period) => nullableNumber(period.start))
      .filter((time): time is number => time !== null);
    const ends = periods
      .map((period) => nullableNumber(period.end))
      .filter((time): time is number => time !== null);
    ...
    const record: CanonicalRecord = {
        ...
        payload: {
          ...
          informedEntities: Array.isArray(value.informed_entity)
            ? value.informed_entity
            : [],
  ...
  const newest = records.map((record) => record.eventTime).filter((time): time is string => time !== undefined).sort().at(-1);
