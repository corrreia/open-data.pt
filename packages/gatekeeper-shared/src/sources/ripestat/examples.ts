...
export const RIPESTAT_EXAMPLES: ExampleFeed[] = [
  example("ripe-portugal-internet-resources-feed", "Internet resources registered to Portugal",
    ...
    { feed: "country-resources", country: "PT" }, 86_400),
  example("ripe-portugal-routing-history-feed", "Portugal internet routing observations",
    ...
    { feed: "country-routing", country: "PT", days: "30" }, 86_400),
...
function network(slug: string, title: string, asn: string, holder: string): ExampleFeed {
  return example(slug, title,
    ...
    { feed: "routing-status", asn }, 28_800);
}

function example(slug: string, title: string, description: string, config: SourceConfig, cadenceSeconds: number): ExampleFeed {
  return {
    slug, title, description, config: { source: "ripestat", ...config },
    publisher: "RIPE NCC", topics: ["internet-infrastructure"], staleAfterSeconds: cadenceSeconds * 3,
    policy: {
      name: "RIPEstat research — republication permission required", version: 1,
      collection: { cadenceSeconds, timeoutSeconds: 60, maxBytes: 1024 * 1024, maxOutputBytes: 2 * 1024 * 1024, maxRecordBytes: 16 * 1024, maxRecords: 5000, historyMode: "changes" },
