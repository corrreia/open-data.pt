import {
  GatekeeperError, contentEtag, field, fixedOrigin, isJsonNumber, isJsonObject, isJsonString,
  lisbonDay, readBoundedJson, resolveFeed, retryAfterSeconds, sourceValidator, streamNdjson,
  type FeedKindDescription, type JsonValue, type NormalizedCollector,
  type NormalizedRow, type ProductFinalization, type SourceConfig, type SourceFetch,
  type SourceValidator, type StreamingTransform, type TransformContext,
} from "../../index";
...
export const REN_PERIODIC_FEEDS = {
  "installed-capacity": {
    kind: "installed-capacity", title: "Installed electricity capacity",
  ...
  "lng-terminal-balance": {
    kind: "lng-terminal-balance", title: "LNG terminal daily balance",
  ...
  "gas-storage": {
    kind: "gas-storage", title: "Underground natural gas storage",
...
function endpoint(origin: string, service: RenPeriodicService, period: string): URL {
  const path = service === "installed-capacity" ? "electricity/ElectricityInstalledPowerMonthly"
    : service === "gas-storage" ? "gas/GasUndergroundStorageBalanceDaily" : "gas/GasLNGTerminalBalanceDaily";
  const url = new URL(`/datahubapi/${path}`, origin);
  url.searchParams.set("culture", "en-US");
  if (service === "installed-capacity") {
    url.searchParams.set("year", period.slice(0, 4)); url.searchParams.set("month", period.slice(5, 7));
...
export async function collectRenPeriodic(options: RenPeriodicOptions, checkpoint: SourceValidator | undefined, signal?: AbortSignal): Promise<SourceFetch> {
  ...
  for (const period of selected) {
    ...
    if ((!response.ok && response.status !== 404) || !response.body) throw new GatekeeperError(`REN periodic API returned HTTP ${response.status}`, "upstream-error", retryAfterSeconds(response.headers));
    ...
    if (isJsonObject(data) && data.message === DAILY_NO_DATA) { missing = true; continue; }
  ...
  return { kind: "body", body, validator, completeness: missing ? "partial" : "complete",
...
function measures(response: JsonValue, service: string): PeriodMeasure[] {
  if (service === "installed-capacity") {
    ...
    for (const item of response) {
      ...
      if (!isJsonNumber(item.monthly_Accumulation) || !Number.isFinite(item.monthly_Accumulation)) throw new GatekeeperError("REN installed capacity value is invalid", "invalid-response");
      result.push({ key: item.type, value: item.monthly_Accumulation, unit: "MW" });
    }
    return result;
  }
  if (!isJsonObject(response) || !isJsonObject(response.Storage) || !isJsonObject(response.Storage.data)
    || !isJsonObject(response.Inputs) || !isJsonObject(response.Outputs) || !Array.isArray(response.Storage.additionalDetails)) {
...
function transformRenPeriodic(body: ReadableStream<Uint8Array>, context: TransformContext): StreamingTransform {
  ...
  async function* rows(): AsyncGenerator<NormalizedRow> {
    for await (const line of streamNdjson(body, { maxElementBytes: 64 * 1024 })) {
      ...
      for (const item of measures(line.response, service ?? "")) {
        ...
        yield { productKey: "periodic", point: { seriesKey: item.key, eventTime, value: item.value, unit: item.unit,
  ...
  return {
    products: [{ productKey: "periodic", slug: context.feed.slug.replace(/-feed$/, ""), title: context.feed.title,
      description: context.feed.description, kind: "series", role: "time-series", updateMode: "source-window", completeness: "complete",
      schema: { fields: [field("seriesKey", "identifier", false), field("eventTime", "datetime", false), field("value", "number", false), field("unit", "category", false), field("dimensions", "json", false)] } }],
    rows: rows(), finish: () => {
