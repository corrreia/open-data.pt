...
export const ROLE = {
  reference: { label: "Reference", short: "Reference", badge: "green", description: "A complete, slow-changing catalog. Every collection replaces the whole set and keeps the previous version." },
  "current-state": { label: "Current state", short: "Current", badge: "blue", description: "The latest known state of each entity. Replaced on every collection; old versions expire quickly." },
  ...
  "time-series": { label: "Time series", short: "Series", badge: "purple", description: "Numeric measurements keyed by series and event time. Points can be corrected later; corrections are logged." },
...
export function buildDatasets(products: Product[], feeds: Feed[]): Dataset[] {
  ...
  for (const feed of feeds) {
    ...
    datasets.push({
      ...
      updatedAt: items.map((product) => product.updatedAt).filter(Boolean).sort().at(-1),
...
export function buildPublishers(datasets: Dataset[]): Publisher[] {
  ...
  for (const dataset of datasets) {
    const publisher: Publisher = publishers.get(dataset.publisherSlug) ?? { name: dataset.publisher, slug: dataset.publisherSlug, datasets: [], topics: new Map(), hosts: new Map(), licences: new Set() };
...
const ROLE_FALLBACK = { reference: "All records", "current-state": "Current records", "event-log": "Events", "time-series": "Series", summary: "Summary" } satisfies { [role in Role]: string };
