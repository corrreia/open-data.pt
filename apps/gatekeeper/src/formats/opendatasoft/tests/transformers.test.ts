...
import { OpendatasoftTransformer, SAMPLE_ROWS } from "#/formats/opendatasoft/transform";

const transformer = new OpendatasoftTransformer();

function fixture(name: string): Uint8Array {
  return new Uint8Array(readFixtureBytes(new URL(`./fixtures/${name}`, import.meta.url)));
}
...
describe("Opendatasoft transformers", () => {
  ...
  it.each<{ fixture: string; slug: string; fields: [string, string][]; eventTime: string }>([
    {
