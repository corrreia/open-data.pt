...
import { NORMALIZED_PROTOCOL, collectNormalized, type CollectionRequest, type JsonObject } from "#/index";
import { feedCollection } from "#/tests/catalog";
import { MYINFO_ORIGIN, collectMyInfoFeed, myInfoPortalUrl, parseFormFields, parseNetwork, parseTrips, parseZones, validateMyInfoFeedConfig } from "#/formats/myinfo/myinfo";
...
function fixture(name: string): string {
  return readFixture(new URL(`./fixtures/${name}.html`, import.meta.url));
}
...
describe("MYINFO Gatekeeper", () => {
  ...
  it("reads the network from one page, and names that page as the source link", async () => {
    ...
    const document = jsonAs<{ feed: string; operator: string; stops: unknown[]; lines: unknown[]; zones: unknown[] }>(await new Response(fetched.body).text());
  ...
  it("searches with the page's own form, its session cookie and midnight, and never selects a line", async () => {
    ...
    if (fetched.kind !== "body") throw new Error("Expected a body");
    const document = jsonAs<{ origin: { name: string }; destination: { name: string }; trips: unknown[] }>(await new Response(fetched.body).text());
