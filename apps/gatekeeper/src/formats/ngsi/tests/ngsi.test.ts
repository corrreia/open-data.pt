...
import { NgsiTransformer } from "#/formats/ngsi/index";

function fixture(path: string): Uint8Array {
  return readFixtureBytes(new URL(`./fixtures/${path}`, import.meta.url));
}
...
describe("the NGSI transformer", () => {
  it("publishes the readings as series and the sensors as a table, with no number in both", () => {
    const result = new NgsiTransformer().transform(fixture("porto-air-quality.json"), context("porto-air-quality-feed", OBSERVATIONS, "observation"));
  ...
  it("names a measure's unit only where the example states one", () => {
    const result = new NgsiTransformer().transform(
      fixture("porto-air-quality.json"),
