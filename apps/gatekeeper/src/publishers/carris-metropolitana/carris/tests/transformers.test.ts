...
import { CarrisTransformer } from "#/publishers/carris-metropolitana/carris/transform";
...
const context = (kind: "alerts" | "lines" | "routes" | "stops" | "vehicles"): TransformContext => ({
  feed: feed(kind),
...
describe("Carris transformers", () => {
  ...
  it("dates an alert by when it takes effect, never by when it was polled", () => {
    ...
    const later = transformer.transform(
      ...
      { ...context("alerts"), observedAt: "2026-09-06T10:05:00Z" },
    );
