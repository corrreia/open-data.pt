...
export const FEED = defineFeed(SNIT_DEPLOYMENT, {
  slug: "snit-pooc-feed",
  config: { feed: "instruments", type: "pooc" },
  policy: SNIT_WEEKLY_POLICY,
  staleAfterSeconds: 2 * WEEK,
  /** Every week: the register's list of municipalities, then every Plano de Ordenamento da Orla Costeira (POOC) in force across them, with the acts behind each. */
