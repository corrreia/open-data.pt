...
export const FEED = defineFeed(SNIT_DEPLOYMENT, {
  slug: "snit-pna-feed",
  config: { feed: "instruments", type: "pna" },
  policy: SNIT_WEEKLY_POLICY,
  staleAfterSeconds: 2 * WEEK,
  /** Every week: the register's list of municipalities, then every Plano Nacional da Água (PNA) in force across them, with the acts behind each. */
