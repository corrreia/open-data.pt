...
export const FEED = defineFeed(SNIT_DEPLOYMENT, {
  slug: "snit-prof-feed",
  config: { feed: "instruments", type: "prof" },
  policy: SNIT_WEEKLY_POLICY,
  staleAfterSeconds: 2 * WEEK,
  /** Every week: the register's list of municipalities, then every Programa Regional de Ordenamento Florestal (PROF) in force across them, with the acts behind each. */
