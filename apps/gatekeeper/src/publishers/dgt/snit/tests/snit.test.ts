...
import { SNIT_API_ORIGIN, SNIT_LANDING_PAGE, SNIT_TYPES, SnitTransformer, collectSnitFeed, resolveSnitFeed, validateSnitFeedConfig } from "#/publishers/dgt/snit/index";

const config = { feed: "instruments", type: "prof" };

const instruments = parseJson(readFixture(new URL("./fixtures/prof-instruments.json", import.meta.url)));
...
describe("SNIT collection", () => {
  ...
  it("answers not-modified when the register returns what it returned last time", async () => {
    ...
    if (etag === undefined) throw new Error("Expected the first read to carry an ETag");
