...
export const FEED = defineFeed(SNIT_DEPLOYMENT, {
  ...
  topics: ["government"],
  config: { feed: "instruments", type: "prot-programa" },
  policy: SNIT_WEEKLY_POLICY,
  staleAfterSeconds: 2 * WEEK,
  /** Every week: the register's list of municipalities, then every Programa Regional de Ordenamento do Território (PROT) in force across them, with the acts behind each. */
