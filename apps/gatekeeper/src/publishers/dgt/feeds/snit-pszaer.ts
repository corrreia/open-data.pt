...
export const FEED = defineFeed(SNIT_DEPLOYMENT, {
  ...
  config: { feed: "instruments", type: "pszaer" },
  policy: SNIT_WEEKLY_POLICY,
  staleAfterSeconds: 2 * WEEK,
  /** Every week: the register's list of municipalities, then every Programa Setorial das Zonas de Aceleração da Implantação de Energias Renováveis (PSZAER) in force across them, with the acts behind each. */
