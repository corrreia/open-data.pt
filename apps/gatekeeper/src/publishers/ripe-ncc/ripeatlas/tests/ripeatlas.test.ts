...
import { collectNormalized, libraryConfig, type JsonObject, type JsonValue, type SourceFetch } from "#/index";
import { collectRipeatlasFeed, RIPEATLAS_ORIGIN, RIPEATLAS_PROBE_FIELDS, validateRipeatlasFeedConfig } from "#/publishers/ripe-ncc/ripeatlas/ripeatlas";
import { RipeatlasTransformer } from "#/publishers/ripe-ncc/ripeatlas/transform";
import { networkBytes, networkContext, networkFixture, networkFrames, networkRequest, networkRows } from "#/tests/networks-support";
...
function fixture(name: string): JsonObject {
  return networkFixture(new URL(`./fixtures/${name}.json`, import.meta.url));
}
...
describe("RIPE Atlas source boundaries", () => {
  ...
  it("walks pages by number until the source drops its continuation", async () => {
    ...
    expect(jsonAs<{ pages: JsonValue[] }>(body).pages).toHaveLength(3);
...
describe("RIPE Atlas anchor inventory normalization", () => {
  it("publishes the hostname RIPE already puts in DNS and no host, address or coordinate", async () => {
    const document = fixture("anchors");
