...
export function validatePeeringdbFeedConfig(config: SourceConfig): SourceConfig {
  ...
  if ((config.feed?.trim() ?? "exchanges") !== "exchanges" || config.country?.trim().toUpperCase() !== "PT")
    throw new GatekeeperError("PeeringDB requires feed=exchanges and country=PT", "invalid-config");
...
export async function collectPeeringdbFeed(config: SourceConfig, apiOrigin: string, fetcher: typeof fetch): Promise<SourceFetch> {
  ...
  base.searchParams.set("country", "PT");
  base.searchParams.set("status", "ok");
  base.searchParams.set("depth", "0");
  base.searchParams.set("fields", PEERINGDB_PUBLIC_FIELDS);
  base.searchParams.set("limit", String(PEERINGDB_PAGE_SIZE));
  ...
  async function* pages(): AsyncGenerator<Uint8Array> {
    ...
    for (let page = 0; page < MAX_REQUESTS; page += 1) {
      ...
      try {
        ...
        throw new GatekeeperError(`PeeringDB request failed: ${error instanceof Error ? error.message : "network failure"}`, "upstream-error");
      }
      if ((response.status >= 300 && response.status < 400) || (response.url && new URL(response.url).origin !== origin))
        throw new GatekeeperError("PeeringDB redirects and unsolicited 304s are not allowed", "source-denied");
      ...
      try {
        ...
        throw new GatekeeperError("PeeringDB page is not JSON", "invalid-response");
      }
      const records = pageRecords(document);
      if (records.length > PEERINGDB_PAGE_SIZE || offset + records.length > PEERINGDB_MAX_RECORDS)
        throw new GatekeeperError("PeeringDB directory exceeds its bounded scope", "response-too-large");
...
export function pageRecords(value: JsonValue | undefined): JsonObject[] {
  if (!isJsonObject(value) || !isJsonArray(value.data) || !value.data.every(isJsonObject))
    throw new GatekeeperError("PeeringDB returned an invalid directory envelope", "invalid-response");
  if (value.meta !== undefined) {
    ...
    if (
      ...
      (meta.next !== undefined && meta.next !== null && meta.next !== "") ||
      (meta.status !== undefined && meta.status !== "ok" && meta.status !== "success" && meta.status !== 200)
    )
      throw new GatekeeperError("PeeringDB reported an API error or unsupported continuation", "invalid-response");
  }
  return value.data;
}
