...
export const FEED = defineFeed(CKAN_DEPLOYMENT, {
  ...
  config: { host: CASCAIS_HOST, dataset: "geocascais-paragemautocarro", resource: "a5375dc1-824a-48ec-b7e3-8030d5543285" },
  policy: CASCAIS_DAILY_REFERENCE,
  staleAfterSeconds: 172_800,
  /** Once a day: the geocascais-paragemautocarro resource on Cascais's CKAN portal, downloaded only when it has changed. */
  fetch: ({ config, validator, library, fetch }) => new CkanSource(library.hosts, fetch).collect(config, validator),
  /** The resource, as the portal serves it, into one table of Cascais's bus stops. */
