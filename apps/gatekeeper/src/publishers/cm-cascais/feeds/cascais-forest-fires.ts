...
export const FEED = defineFeed(CKAN_DEPLOYMENT, {
  slug: "cascais-forest-fires-feed",
  ...
  config: { host: CASCAIS_HOST, dataset: "geocascais-incendioflorestal", resource: "062d1d34-dce0-4a5a-bae1-99cc7f124bcf" },
  policy: CASCAIS_DAILY_REFERENCE,
  staleAfterSeconds: 172_800,
  /** Once a day: the geocascais-incendioflorestal resource on Cascais's CKAN portal, downloaded only when it has changed. */
  fetch: ({ config, validator, library, fetch }) => new CkanSource(library.hosts, fetch).collect(config, validator),
  /** The resource, as the portal serves it, into one table of Cascais's forest fire areas. */
