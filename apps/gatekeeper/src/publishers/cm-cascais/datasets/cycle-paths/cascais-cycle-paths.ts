...
export const FEED = defineFeed(CKAN_DEPLOYMENT, {
  slug: "cascais-cycle-paths-feed",
  config: { host: CASCAIS_HOST, dataset: "geocascais-ciclovia", resource: "74649057-6245-4214-a68c-d4acc3753b7d" },
  policy: CASCAIS_DAILY_REFERENCE,
  staleAfterSeconds: 172_800,
  /** Once a day: the geocascais-ciclovia resource on Cascais's CKAN portal, downloaded only when it has changed. */
  fetch: ({ config, validator, library, fetch }) => new CkanSource(library.hosts, fetch).collect(config, validator),
  /** The resource, as the portal serves it, into one table of Cascais's cycle paths. */
