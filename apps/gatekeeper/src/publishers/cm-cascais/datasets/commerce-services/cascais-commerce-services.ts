...
export const FEED = defineFeed(CKAN_DEPLOYMENT, {
  slug: "cascais-commerce-services-feed",
  config: { host: CASCAIS_HOST, dataset: "geocascais-comercioservico", resource: "1f30c5fa-acb7-4cab-859e-f0e549999609" },
  policy: CASCAIS_DAILY_REFERENCE,
  staleAfterSeconds: 172_800,
  /** Once a day: the geocascais-comercioservico resource on Cascais's CKAN portal, downloaded only when it has changed. */
  fetch: ({ config, validator, library, fetch }) => new CkanSource(library.hosts, fetch).collect(config, validator),
  /** The resource, as the portal serves it, into one table of Cascais's commerce and services. */
