...
as each publisher's `logo`. Each lives in its publisher's folder, beside the `index.ts`
that names them, as `<key>/logo.svg` or `<key>/logo.png`; the site's build copies each one
to `dist/publishers/<key>.<ext>`, which is where the API says it is. The `logo` field of
the folder's `PUBLISHER` says whether there is one and with what extension, so nothing
else has to be stored. `tests/publisher-logos.test.ts` holds the two to each other.
