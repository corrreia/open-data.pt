...
import { runTransformer, type JsonObject, type JsonValue, type TransformContext } from "#/index";
import type { RenServiceName } from "#/publishers/ren/ren/ren";
import { RenTransformer } from "#/publishers/ren/ren/transform";

function fixture(name: string): Uint8Array {
  return readFixtureBytes(new URL(`./fixtures/${name}.json`, import.meta.url));
}
...
function parsedFixture(name: string): JsonObject {
  return jsonAs<JsonObject>(readFixture(new URL(`./fixtures/${name}.json`, import.meta.url)));
}
...
describe("REN transformers", () => {
  ...
  it("rolls post-midnight gas hours into the next civil day", () => {
    // The gas-consumption fixture is a REN chart document with the service, days and chart axes this test walks.
    const value = jsonAs<{
      service: string;
      days: Array<{
        day: string;
    ...
    }>(readFixture(new URL("./fixtures/gas-consumption.json", import.meta.url)));
    const [first] = value.days;
    if (!first) throw new Error("The gas-consumption fixture holds no day");
    first.response.xAxis.categories = ["23:00", "00:00"];
    for (const series of first.response.series) {
