...
import { MetroLisboaTransformer, bracketList, intervalSeconds, metroTimestamp } from "#/publishers/metropolitano-de-lisboa/metrolisboa/transform";
...
function resposta(name: string): JsonValue {
  return jsonAs<JsonObject>(readFixture(new URL(`./fixtures/${name}.json`, import.meta.url))).resposta ?? null;
}
...
function normalize(value: JsonObject) {
  return transformer.transform(document(value));
}

describe("Metro Lisboa normalizer", () => {
  it("reads the four lines' status, trimmed, with the operator's short state as written", () => {
    const result = normalize({ feed: "line-status", status: resposta("line-status") });
  ...
  it("reports a line closed for the night as closed, not as disrupted", () => {
    ...
    const records = normalize({ feed: "line-status", status }).products[0]?.records;
  ...
  it("keeps a disruption's message, short state and message type", () => {
    ...
    const record = normalize({ feed: "line-status", status }).products[0]?.records?.[0];
  ...
  it("turns every platform's next trains into one record dated by the operator's own clock, with its destination's name", () => {
    ...
    const result = normalize({ feed: "waiting-times", waiting, destinations: resposta("destinations") });
    const product = result.products[0];
  ...
  it("rejects malformed and repeated platforms, keeps an unknown destination without a name, and reads missing trailing trains as empty", () => {
    ...
    const result = normalize({ feed: "waiting-times", waiting, destinations: [{ id_destino: "33", nome_destino: "Reboleira" }] });
  ...
  it("reads the empty list served outside service hours as a complete, empty snapshot", () => {
    const product = normalize({ feed: "waiting-times", waiting: [], destinations: [] }).products[0];
  ...
  it("reads stations' coordinates as numbers and their bracketed lists as lists", () => {
    const result = normalize({ feed: "stations", stations: resposta("stations") });
    ...
    const bad = normalize({ feed: "stations", stations: [{ stop_id: "XX", stop_name: "Nowhere", stop_lat: "91", stop_lon: "0" }] });
  ...
  it("reads each headway band with its interval in seconds, keyed by line, day type and start", () => {
    ...
    const result = normalize({ feed: "headways", headways: [{ line: "amarela", day: "S", rows }] });
    ...
    expect(() => normalize({ feed: "headways", headways: [{ line: "rosa", day: "S", rows: [] }] })).toThrow(/unknown line/);
  });
  ...
  it("is deterministic and independent of when it runs", () => {
    ...
    // The transformer takes no clock and no context: only the bytes it is given.
    const early = transformer.transform(document(value));
    const late = transformer.transform(document(value));
