...
function object(value: JsonValue | undefined): JsonObject {
  if (!isJsonObject(value)) throw new Error("Expected fixture object");
  return value;
}
function array(value: JsonValue | undefined): JsonValue[] {
  if (!isJsonArray(value)) throw new Error("Expected fixture array");
  return value;
}
...
async function run(feed: string, value = fixture(feed), chunkSize = 1, observedAt?: string): Promise<Result> {
  return runText(feed, JSON.stringify(value), chunkSize, observedAt);
}
async function runText(feed: string, text: string, chunkSize = 1, observedAt?: string): Promise<Result> {
  const transformed = transformParliament(body(text, chunkSize), context(feed, observedAt));
  ...
  for await (const row of transformed.rows) {
    ...
    const values = records.get(row.productKey) ?? [];
    values.push(row.record);
    records.set(row.productKey, values);
  }
...
describe("Parliament streaming normalization", () => {
  ...
  it("publishes useful career additions only and excludes private/sensitive fields", async () => {
    ...
    Object.assign(object(data[0]), {
      CadDtNascimento: "PRIVATE_SENTINEL",
      CadSexo: "PRIVATE_SENTINEL",
      private_contact: "PRIVATE_SENTINEL",
      CadDeputadoLegis: [{ private: "PRIVATE_SENTINEL" }],
    });
  ...
  it("keeps petition count/date semantics without copying authors or private nested documents", async () => {
    ...
    first.PetAutor = "PRIVATE_SENTINEL";
    first.Documentos = [{ body: "PRIVATE_SENTINEL" }];
    ...
    expect(records?.[0]).toMatchObject({
      ...
      payload: { signatures: 1234, initial_signatures: 1000, entry_date: "2026-06-15" },
    });
  ...
  it("dates legislation from actual publications, never its numeric year or the poll time", async () => {
    const result = await run("diplomas");
    const records = result.records.get("diplomas");
    expect(records?.[0]).toMatchObject({
      eventTime: "2026-06-20T00:00:00.000Z",
      sourcePublishedAt: "2026-06-22T00:00:00.000Z",
      payload: { text_url: "https://app.parlamento.pt/example-law.pdf", civil_year: "2026" },
    });
    expect(records?.[1]?.eventTime).toBeUndefined();
    expect(records?.[1]?.sourcePublishedAt).toBeUndefined();
    const data = array(fixture("diplomas"));
    object(data[0]).LinkTexto = "javascript:alert(1)";
  ...
  it("preserves scoped committee membership history and Lisbon meeting clocks, with honest incomplete sub-products", async () => {
    ...
    expect(result.records.get("meetings")?.[0]).toMatchObject({
      entityKey: "70000001:90000001",
      eventTime: "2026-07-08T13:30:00.000Z",
      payload: { starts_at: "2026-07-08T13:30:00.000Z", source_start: "2026-07-08T14:30:00" },
    });
  ...
  it("rejects duplicate identities, wrong-legislature payloads and incomplete required arrays", async () => {
    const petitions = array(fixture("petitions"));
    petitions.push(petitions[0] ?? null);
    await expect(run("petitions", petitions)).rejects.toThrow("repeated");
    const members = object(fixture("members"));
    object(array(members.Deputados)[0]).LegDes = "XVI";
    await expect(run("members", members)).rejects.toThrow("legislature");
    const activities = object(fixture("activities"));
    delete activities.Eventos;
  ...
  it("enforces element and unrelated-envelope byte bounds", async () => {
    const data = array(fixture("petitions"));
    object(data[0]).unselected = "x".repeat(PARLIAMENT_ELEMENT_BYTES);
    await expect(run("petitions", data, 8192)).rejects.toMatchObject({ code: "response-too-large" });
    const activities = object(fixture("activities"));
    activities.unrelated = "x".repeat(2 * 1024 * 1024);
