...
import { NORMALIZED_PROTOCOL, collectNormalized, isJsonObject, isNormalizedFrame, parseJson, type CollectionRequest, type NormalizedRow, type TransformContext } from "#/index";
import { collectIpmaFeed, IPMA_FEEDS, validateIpmaFeedConfig } from "#/publishers/ipma/ipma/ipma";
import { resolveIpmaFeed } from "#/publishers/ipma/ipma/collector";
import { IpmaDatasetTransformer, type IpmaDatasetFeed } from "#/publishers/ipma/ipma/datasets";
import { feedCollection } from "#/tests/catalog";

const transformer = new IpmaDatasetTransformer();
const ORIGIN = "https://api.ipma.pt";
const fixture = (name: string) => readFixture(new URL(`./fixtures/${name}`, import.meta.url));
