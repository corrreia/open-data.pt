...
/**
 * The JSON body a response carries, at the shape the caller names. It reads the text, so it takes the Workers
 * runtime's `Response` and the one wrangler's test harness hands back through Node alike.
 */
export async function jsonBody<T>(response: Pick<Response, "text">): Promise<T> {
  return jsonAs<T>(await response.text());
}
