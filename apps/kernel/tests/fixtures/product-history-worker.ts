...
/**
 * The lake the kernel is typed to read (its generated `Env` holds wrangler.jsonc's literal vars). Nothing reaches it:
 * every query goes to the fixture below, and any other URL fails the test.
 */
const LAKE: Pick<Env, "CATALOG_TOKEN" | "CLOUDFLARE_ACCOUNT_ID" | "LAKE_BUCKET"> = {
  CATALOG_TOKEN: "test-only",
...
export default class ProductHistoryTestWorker extends KernelWorker {
  ...
  protected override apiContext(): ApiContext {
    return {
      ...super.apiContext(),
      lakeQueryFetch: async (input, init) => {
        if (String(input) !== `https://api.sql.cloudflarestorage.com/api/v1/accounts/${LAKE.CLOUDFLARE_ACCOUNT_ID}/r2-sql/query/${LAKE.LAKE_BUCKET}`)
...
export class HistoryFixture extends DurableObject {
  constructor(ctx: DurableObjectState, env: HistoryTestEnv) {
    ...
    for (const row of [
      ...
      sql.exec(
        "INSERT INTO records VALUES (?, ?, ?, ?, 'upsert', ?, NULL, NULL, NULL, NULL, ?, ?, ?, ?)",
        row.owner,
        row.slug,
        `rev-${row.id}`,
        row.id,
        ...
        JSON.stringify({
          value: row.value,
          __openDataSourceSequence: `seq-${row.id}`,
          id: "spoof",
          operation: "delete",
          eventTime: "1900-01-01T00:00:00Z",
          sourcePublishedAt: "spoof",
          observedAt: "spoof",
          ingestedAt: "spoof",
        }),
        day(row.observed),
    ...
    for (const row of [
      ...
      sql.exec(
        "INSERT INTO points VALUES (?, ?, ?, 'load', ?, ?, 'MW', '{}', ?, ?)",
        row.owner,
        row.slug,
        row.revision,
        day(row.time),
  ...
  override async fetch(request: Request): Promise<Response> {
    ...
    if (query.includes("series_key = 'provider-failure'"))
      return Response.json({ success: false, errors: [{ message: "fixture query failure" }], result: { metrics: { bytes_scanned: 207 } } }, { status: 500 });
