...
describe.skipIf(ROWS === 0)("one large product at scale", () => {
  it(`collects ${ROWS} rows with bounded heap, then turns 1% changes into 1% revisions`, async () => {
    ...
    const record = (index: number, value: string) => ({
      entityKey: `entity-${String(index).padStart(8, "0")}`,
      payload: { name: `Entity ${index}`, value, district: `D${index % 20}`, lat: 38 + (index % 1000) / 10_000, lon: -9 - (index % 700) / 10_000 },
    });
    let peak = 0;
    const baseline = liveHeap();
    const sampler = setInterval(() => {
      peak = Math.max(peak, liveHeap() - baseline);
    }, 250);
    const measure = async (value: (index: number) => string) => {
      h.source.generate = function* () {
        for (let index = 0; index < ROWS; index += 1) yield record(index, value(index));
      };
    ...
    try {
      ...
      const report = {
        ...
        baseline: first,
        onePercent: second,
        lakeRows: h.lakeCount,
        manifestChunks: h.entry()?.rowCount === ROWS ? "all rows served" : h.entry()?.rowCount,
      };
