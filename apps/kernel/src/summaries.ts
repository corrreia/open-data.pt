...
/** The latest revision of each point with an event time in the day, by series and UTC hour. */
...
function lateHourlySql(ingest: DayBounds, from: string, before: string, slug: string | undefined, last: LakeBucket | undefined): string {
  return `WITH buckets AS (SELECT product_slug, series_key, date_trunc('hour', event_time) AS hour, COUNT(DISTINCT event_time) AS n, AVG(value) AS mean, MIN(value) AS low, MAX(value) AS high, MAX(unit) AS unit FROM open_data.points WHERE __ingest_ts >= TIMESTAMP '${ingest.from}' AND __ingest_ts < TIMESTAMP '${ingest.to}' AND event_time >= TIMESTAMP '${from}' AND event_time < TIMESTAMP '${before}'${afterProduct(slug)} GROUP BY product_slug, series_key, date_trunc('hour', event_time)) SELECT product_slug, series_key, hour, n, mean, low, high, unit FROM buckets${keyset("hour", last)} ORDER BY product_slug, series_key, hour LIMIT ${LATE_PAGE + 1}`;
}
...
function lateDailySql(ingest: DayBounds, before: string, slug: string | undefined, last: LakeBucket | undefined): string {
  return `WITH shifted AS (SELECT product_slug, series_key, event_time, value, unit, ${lisbonWallClockSql(DST_FROM_YEAR, Number(before.slice(0, 4)))} AS local_time FROM open_data.points WHERE __ingest_ts >= TIMESTAMP '${ingest.from}' AND __ingest_ts < TIMESTAMP '${ingest.to}' AND event_time < TIMESTAMP '${before}'${afterProduct(slug)}), buckets AS (SELECT product_slug, series_key, date_trunc('day', local_time) AS day, COUNT(DISTINCT event_time) AS n, AVG(value) AS mean, MIN(value) AS low, MAX(value) AS high, MAX(unit) AS unit FROM shifted GROUP BY product_slug, series_key, date_trunc('day', local_time)) SELECT product_slug, series_key, day, n, mean, low, high, unit FROM buckets${keyset("day", last)} ORDER BY product_slug, series_key, day LIMIT ${LATE_PAGE + 1}`;
}
